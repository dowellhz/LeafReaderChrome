(() => {
  ...
  const paintRange = (record, range) => {
    ...
    const name = {
      ...
      analysis: 'leafreader-page-analysis',
  ...
  async function handleAction(action) {
    ...
    if (action === 'word') {
      ...
      try {
        const offline = await LeafDictionary.lookup(selectedText);
        if (offline) { const answer = LeafDictionary.markdown(offline); await saveDefinition(selectedText, answer, { conversationId, presentation:'dictionary' }); display('单词释义', answer, selectedText, { conversationId, presentation:'dictionary' }); clearSelection(); return; }
    ...
    const conversationId = crypto.randomUUID(); const kind = action === 'translate' ? 'translation' : action === 'sentence' ? 'analysis' : 'explanation'; const marker = await saveSelectionMarker(kind, { conversationId, presentation:'chat' }); addVisualHighlight(marker);
    const title=action === 'translate' ? 'Translation' : action === 'sentence' ? '难句分析' : '讲解';display(title,'Thinking…',selectedText,{ conversationId });try { let answer; if (action === 'translate') { const prepared=LeafTranslation.prepareParagraphTranslation(selectedText); const instruction=prepared.count > 1 ? `Translate every sentence in the selected text completely and in order. The input has ${prepared.count} paragraphs labeled [[P1]] through [[P${prepared.count}]]. Keep every label unchanged and in the same order, translate each paragraph separately, and put one blank line between labeled paragraphs. Do not merge, summarize, omit, or add commentary.` : 'Translate every sentence in the selected text completely and in order. Do not summarize, omit, or add commentary. Return only the translation.'; answer=LeafTranslation.restoreParagraphTranslation(await askAI(instruction,prepared.text),prepared.count); } else if (action === 'sentence') answer=await askAI('Analyze this difficult sentence for a Chinese learner. Use concise Markdown sections: ## 句子骨架 (main clause), ## 从句与修饰关系, ## 难点, and ## 分层翻译. Keep the explanation anchored to this exact sentence and do not add unsupported context.'); else answer=await askAI('Explain the meaning in context, useful vocabulary or grammar, and the author’s likely intent.'); display(title,answer,selectedText,{ conversationId });} catch(error) {display(title,`Could not reach the AI provider: ${error.message}`,selectedText,{ conversationId });}clearSelection();
  }
  ...
  const openMarkedRecord = (record) => {
    const title = { translation:'Translation', dictionary:'单词释义', analysis:'难句分析', explanation:'讲解', note:'Note', word:'单词释义', highlight:'Highlight' }[record.kind] || 'LeafReader';
